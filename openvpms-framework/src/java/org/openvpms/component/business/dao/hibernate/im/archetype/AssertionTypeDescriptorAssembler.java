...
/**
 * An {@link Assembler} responsible for assembling {@link AssertionTypeDescriptorDO} instances from
 * {@link AssertionTypeDescriptor}s and vice-versa.
 *
 * @author Tim Anderson
 */
public class AssertionTypeDescriptorAssembler
        extends IMObjectAssembler<AssertionTypeDescriptor, AssertionTypeDescriptorDO> {

    /**
     * Assembles action types.
     */
    private static final SetAssembler<ActionTypeDescriptor, ActionTypeDescriptorDO> TYPES
    ...
    protected void assembleDO(AssertionTypeDescriptorDO target, AssertionTypeDescriptor source, DOState state,
                              Context context) {
        super.assembleDO(target, source, state, context);
        target.setPropertyArchetype(source.getPropertyArchetype());
        TYPES.assembleDO(target.getActionTypes(), (Set<ActionTypeDescriptor>) (Set) source.getActionTypes(),
                         state, context);
    }
    ...
    @Override
    @SuppressWarnings("unchecked")
    protected void assembleObject(AssertionTypeDescriptor target, AssertionTypeDescriptorDO source, Context context) {
        super.assembleObject(target, source, context);
        target.setPropertyArchetype(source.getPropertyArchetype());
        TYPES.assembleObject((Set<ActionTypeDescriptor>) (Set) target.getActionTypes(), source.getActionTypes(),
                             context);
    }
