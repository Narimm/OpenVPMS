 ...
 * <p/>
 * This supports sorting on virtual sort nodes, and filtering on the archetype node, if a template has it.
 *
 * @author Tim Anderson
 */
public abstract class AbstractDocumentTemplateQuery extends AbstractIMObjectQuery<Entity> {
    ...
    public ResultSet<Entity> query(SortConstraint[] sort) {
        ResultSet<Entity> result = new LocalSortResultSet<>(super.query(null));
        result.sort(sort);
    ...
    public boolean selects(IMObjectReference reference) {
        if (types.length == 0) {
            ResultSet<Entity> set = query(); // to avoid wrapping the set in LocalSortResultSet
