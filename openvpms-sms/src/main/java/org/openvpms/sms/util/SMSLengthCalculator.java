...
/**
 * Helper to calculate lengths of SMS messages.
 *
 * @author Tim Anderson
 */
public class SMSLengthCalculator {
    ...
    /**
     * GSM 03.38 to Unicode, derived from http://unicode.org/Public/MAPPINGS/ETSI/GSM0338.TXT
     * <br/>
     * The first element of each pair is the ETSI GSM 03.38 7-bit default alphabet code (in hex as 0xXX, or 0xXXXX for
     * double-byte sequences).
     * <br/>
     * The second element is the Unicode scalar value (in hex as 0xXXXX)
     */
    public static final char[][] GSM_TO_UNICODE = {
    ...
    /**
     * Maximum no. of characters for a single part message encoded using GSM characters.
     */
    public static final int SINGLE_PART_GSM_LENGTH = 160;

    /**
     * Maximum no. of characters for a multi-part message encoded using GSM characters.
     */
    public static final int MULTI_PART_GSM_LENGTH = 153;

    /**
     * Maximum no. of characters for a single part message encoded using UCS-2 characters.
     */
    public static final int SINGLE_PART_UNICODE_LENGTH = 70;

    /**
     * Maximum no. of characters for a for a multi-part message using UCS-2 characters.
     */
    ...
    public static int getParts(String message) {
        int parts;
        int length = getGSMLength(message);
        if (length == -1) {
            // unicode message
            length = message.length();
            parts = getParts(length, SINGLE_PART_UNICODE_LENGTH, MULTI_PART_UNICODE_LENGTH);
        } else {
            parts = getParts(length, SINGLE_PART_GSM_LENGTH, MULTI_PART_GSM_LENGTH);
        }
        return parts;
    }
     ...
     * @param isGSM if {@code true}, the message will be encoded as GSM (7-bit), otherwise it will be encoded as UCS-2
     * @return the maximum length
     */
    public static int getMaxLength(int parts, boolean isGSM) {
        return (isGSM) ? getMaxLength(parts, SINGLE_PART_GSM_LENGTH, MULTI_PART_GSM_LENGTH)
                       : getMaxLength(parts, SINGLE_PART_UNICODE_LENGTH, MULTI_PART_UNICODE_LENGTH);
    }

    /**
     * Returns the maximum length for a message.
     *
     * @param parts            the message parts
     * @param singlePartLength the length for a single part message
     * @param multiPartLength  the length for a multi-part message
     * @return the maximum length
     */
     ...
     * @param singlePartLength the maximum length for single messages
     * @param multiPartLength  the maximum length per part for a multi-part message
     * @return the number of parts
     */
