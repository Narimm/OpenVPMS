...
public class OrderMapper {
    ...
    public OrderMapper(IArchetypeService service, ILookupService lookupService) {
        ...
        productRules = new ProductRules(service);
    }
    ...
    public OrderType map(FinancialAct order) {
        ...
        MonetaryTotalType total = getMonetaryTotal(order.getTotal());
        ...
        for (Act item : bean.getNodeActs("items")) {
            OrderLineType line = getOrderLine(item, supplier);
     ...
     */
    private MonetaryTotalType getMonetaryTotal(BigDecimal payableAmount) {
    ...
    private AddressType getAddress(Contact contact) {
        ...
        String city = LookupHelper.getName(service, lookupService, contact, "suburb");
        CityNameType cityName = initName(new CityNameType(), city);

        String state = LookupHelper.getName(service, lookupService, contact, "state");
