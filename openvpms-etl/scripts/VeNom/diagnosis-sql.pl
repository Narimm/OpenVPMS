...
select 0, UUID(), 'lookup.diagnosisVeNom', 1, '1.0', 'VENOM-$id', '$term', '$modelling', 0
from dual
where not exists (select * from lookups e where e.arch_short_name = 'lookup.diagnosisVeNom' and e.code = 'VENOM-$id');\n";
     print "insert into lookup_details (lookup_id, name, type, value)
select l.lookup_id, 'dataDictionaryId', 'string', '$id'
from lookups l
where l.arch_short_name = 'lookup.diagnosisVeNom' and l.code = 'VENOM-$id'
